...
BEGIN_TEST(test_CallNonGenericMethodOnProxy)
{
  ...
  JS::RootedObject globalA(cx, JS_NewGlobalObject(cx, getGlobalClass(), NULL, JS_ZONE_CHROME));
  CHECK(globalA);
  ...
  {
    JS::RootedObject globalB(cx, JS_NewGlobalObject(cx, getGlobalClass(), NULL, JS_ZONE_CHROME));
    CHECK(globalB);
