...
BEGIN_TEST(testDebugger_debuggerObjectVsDebugMode)
{
    ...
    JS::RootedObject debuggee(cx, JS_NewGlobalObject(cx, getGlobalClass(), NULL, JS_ZONE_CHROME));
    CHECK(debuggee);
...
BEGIN_TEST(testDebugger_newScriptHook)
{
    ...
    CHECK(JS_DefineDebuggerObject(cx, global));
    JS::RootedObject g(cx, JS_NewGlobalObject(cx, getGlobalClass(), NULL, JS_ZONE_CHROME));
    CHECK(g);
    {
        JSAutoEnterCompartment ae;
        CHECK(ae.enter(cx, g));
