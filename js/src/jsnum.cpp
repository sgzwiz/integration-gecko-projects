...
ComputeAccurateDecimalInteger(JSContext *cx, const jschar *start, const jschar *end, double *dp)
{
    ...
    char *estr;
    int err = 0;
    *dp = js_strtod_harder(cx->thread()->dtoaState, cstr, &estr, &err);
       ...
       CallArgs args)
{
    ...
    numStr = js_dtostr(cx->thread()->dtoaState, buf, sizeof buf,
...
FracNumberToCString(JSContext *cx, ToCStringBuf *cbuf, double d, int base = 10)
{
    ...
    if (base == 10) {
        ...
        numStr = cbuf->dbuf = js_dtobasestr(cx->thread()->dtoaState, base, d);
    }
    return numStr;
}
          ...
          const jschar **ep, double *dp)
{
    ...
    if (*istr == 'I' && !strncmp(istr, "Infinity", 8)) {
        ...
        int err;
        d = js_strtod_harder(cx->thread()->dtoaState, cstr, &estr, &err);
        if (d == HUGE_VAL)
