        ...
        if (cx->isExceptionPending()) {
            ...
            if (handler || !cx->compartment->getDebuggees().empty()) {
                ...
                case JSTRAP_RETURN:
                    ...
                    return cx->compartment->jaegerCompartment().forceReturnFromExternC();
...
stubs::ScriptDebugPrologue(VMFrame &f)
{
    ...
    switch (status) {
      case JSTRAP_CONTINUE:
        break;
      case JSTRAP_RETURN:
        *f.returnAddressLocation() = f.cx->compartment->jaegerCompartment().forceReturnFromFastCall();
        return;
      case JSTRAP_ERROR:
      case JSTRAP_THROW:
        THROW();
        ...
        if (script->debugMode) {
            ...
            switch (status) {
              case JSTRAP_CONTINUE:
                break;
              case JSTRAP_RETURN:
                *f.returnAddressLocation() = f.cx->compartment->jaegerCompartment().forceReturnFromExternC();
                return NULL;
              case JSTRAP_THROW:
              case JSTRAP_ERROR:
                return js_InternalThrow(f);
