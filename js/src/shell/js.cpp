...
{
    RootedObject obj(cx, JS_NewGlobalObject(cx, &sandbox_class, NULL, JS_ZONE_CHROME));
    if (!obj)
        return NULL;
...
NewGlobalObject(JSContext *cx)
{
    RootedObject glob(cx, JS_NewGlobalObject(cx, &global_class, NULL, JS_ZONE_CHROME));
    if (!glob)
        return NULL;
