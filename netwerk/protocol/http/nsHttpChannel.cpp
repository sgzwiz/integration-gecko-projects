...
nsHttpChannel::OpenCacheEntry(bool usingSSL)
{
    ...
    rv = cacheStorage->AsyncOpenURI(
      openURI, nsPrintfCString("%d", mPostID), cacheEntryOpenFlags, this);
    NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsHttpChannel::OnCacheEntryCheck(nsICacheEntry* entry, nsIApplicationCache* appCache, bool *valid)
{
    //AssertOnCacheThread();
    ...
    *valid = false;
    ...
    /* mayhemer: For w/o (new) entries check is not called
    // Don't proceed unless we have opened a cache entry for reading.
    if (!entry || !(mCacheEntryIsWriteOnly & nsICache::ACCESS_READ))
        return NS_OK;
    */
    ...
    // !! mayhemer: read-only, unless they are read-only because of INHIBIT_CACHING
    // or because
    ...
    if (!mApplicationCacheForWrite &&
        ...
        if (NS_SUCCEEDED(rv)) {
            *valid = (mCachedContentIsValid = true);
    ...
    *valid = (mCachedContentIsValid = !doValidation);
    ...
    if (mCachedContentIsValid || mDidReval) {
        ...
        if (NS_FAILED(rv)) {
            ...
            *valid = (mCachedContentIsValid = false);
        }
    }
