...
private:
  ...
  static bool
  IsVersionChangeEvent(JS::Handle<JS::Value> v)
  {
    if (!v.isObject())
      return false;
    JSObject* obj = &v.toObject();
    return JS_GetClass(obj) == &sClass &&
           GetJSPrivateSafeish<VersionChangeEvent>(obj) != nullptr;
  }
  ...
  struct Property
  {
    ...
    Get(JSContext* aCx, unsigned aArgc, JS::Value* aVp)
    {
      static_assert(SLOT_FIRST <= Slot && Slot < SLOT_COUNT, "bad slot");
      JS::CallArgs args = JS::CallArgsFromVp(aArgc, aVp);
      return JS::CallNonGenericMethod<IsVersionChangeEvent, GetPropertyImpl<Slot> >(aCx, args);
    }
  };
};

const JSClass VersionChangeEvent::sClass = {
...
const JSPropertySpec VersionChangeEvent::sProperties[] = {
  JS_PSGS("oldVersion", Property<SLOT_oldVersion>::Get, GetterOnlyJSNative,
          JSPROP_ENUMERATE),
  JS_PSGS("newVersion", Property<SLOT_newVersion>::Get, GetterOnlyJSNative,
          JSPROP_ENUMERATE),
  JS_PS_END
};
