                          ...
                          ErrorResult& aRv)
{
  ...
  nsRefPtr<DataTransfer> transfer = new DataTransfer(eventType, aIsExternal,
                                                     -1);
...
{
  nsCOMPtr<nsIVariant> data;
  aRv = nsContentUtils::XPConnect()->JSValToVariant(aCx,
                                                    const_cast<jsval*>(aData.address()),
                                                    getter_AddRefs(data));
  if (!aRv.Failed()) {
                    ...
                    DataTransfer** aNewDataTransfer)
{
  DataTransfer* newDataTransfer =
    new DataTransfer(aEventType, mEffectAllowed, mCursorState,
